...
- `DEMO_NL2SQL_HTTP_TIMEOUT_SECONDS` (timeout HTTP demo→API)
- `DEMO_MYSQL_WAIT_TIMEOUT_SECONDS` (wait timeout del MySQL demo)
- `DEMO_MYSQL_INTERACTIVE_TIMEOUT_SECONDS` (interactive timeout del MySQL demo)
- `DEMO_MYSQL_MAX_EXECUTION_TIME_MS` (límite de ejecución por statement en MySQL demo)
- `DEMO_HEALTHCHECK_TIMEOUT_SECONDS` (timeout del healthcheck de Compose)
