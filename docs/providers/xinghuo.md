...
$ch = curl_init(getenv('LLM_BASE_URL') ?: 'https://spark-api-open.xf-yun.com/v1/chat/completions');
curl_setopt_array($ch, [CURLOPT_POST=>true, CURLOPT_RETURNTRANSFER=>true,
  CURLOPT_HTTPHEADER=>['Content-Type: application/json','Authorization: Bearer '.getenv('XINGHUO_API_KEY')],
...
### Node.js (LTS actual)
```js
const r = await fetch(process.env.LLM_BASE_URL ?? 'https://spark-api-open.xf-yun.com/v1/chat/completions', {
  method: 'POST',
  headers: {'Content-Type':'application/json','Authorization':`Bearer ${process.env.XINGHUO_API_KEY}`},
...
import os, requests
resp = requests.post(
    os.getenv('LLM_BASE_URL') or 'https://spark-api-open.xf-yun.com/v1/chat/completions',
    headers={'Authorization': f"Bearer {os.getenv('XINGHUO_API_KEY')}", 'Content-Type': 'application/json'},
...
var res = await http.PostAsync(Environment.GetEnvironmentVariable("LLM_BASE_URL") ?? "https://spark-api-open.xf-yun.com/v1/chat/completions", new StringContent(body, Encoding.UTF8, "application/json"));
...
var req = HttpRequest.newBuilder(URI.create(System.getenv().getOrDefault("LLM_BASE_URL", "https://spark-api-open.xf-yun.com/v1/chat/completions")))
